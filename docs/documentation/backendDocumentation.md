...
### Diagram
![MVP Database Structure](./images/MVP%20Database%20Structure.png)
...
- `transactionAmount` is a `double` to support decimal values since transactions can contain pence, for now we are assuming only one form of currency to meet MVP standards.
...
After reviewing the options for creating the database, we settled on Supabase. First, we created an appropriate organisation and project on Supabase, followed by creating our table:

![Creation](images/Transactions1.png)

In the table, we put the fields we planned during the design of the database, following the types and restrictions we planned:

![Design](images/Transactions2.png)

As the project MVP does not require a full authentication system, we made this table allow anonymous access, so that data can be written to and read from it without using the Supabase authentication system:

![Anonymous Access](images/Auth3.png)

Finally, to meet MVP requirements, we populated the table with some very basic data, with the `transactionID` being a randomly generated UID:

![Data](images/Random4.png)
---

# Creating the Cronjob

We have previously created cronjobs in our first year of programming at Nottingham. We will again be using [cron-job.org](https://cron-job.org) for our cronjob:

![Cron job](images/Cronjob5.png)
